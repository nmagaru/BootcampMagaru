...
public class SalesDao {
    ...
    public void addSalesContract(SalesContract contract) {
        String query =
                "INSERT INTO sales_contracts (name, email, phone, date, base_price, vin, financed, total_price, monthly_payment) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try(Connection conn = DriverManager.getConnection(connectionString, userName, password);
            ...
            stmt.setBoolean(7, contract.isFinanced());
            stmt.setDouble(8, contract.getTotalPrice());
            stmt.setDouble(9, contract.getMonthlyPayment());
